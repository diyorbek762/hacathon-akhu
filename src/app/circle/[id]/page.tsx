...
const circles = [
  { id: '1', emoji: '🤖', name: 'AI Research Collective', desc: 'Weekly paper readings and ML discussions.', members: 2840, creator: 'Mia Chen' },
  { id: '2', emoji: '🚀', name: 'Founders & Builders', desc: 'Student entrepreneurs building startups.', members: 1650, creator: 'James Park' },
  { id: '3', emoji: '💪', name: '5AM Gym Squad', desc: 'Early risers hitting the gym before 7am.', members: 412, creator: 'Sofia Rodriguez' },
  { id: '4', emoji: '🧪', name: 'Research Nexus', desc: 'Undergrad research collaborations.', members: 980, creator: 'Priya Sharma' },
  { id: '5', emoji: '🎨', name: 'Creative Tech Collective', desc: 'Design meets engineering.', members: 730, creator: 'Zara Ahmed' },
  { id: '6', emoji: '♟️', name: 'Chess & Strategy Club', desc: 'Tournaments and strategy workshops.', members: 295, creator: 'Luca Moretti' },
]
...
export default function CircleChannelPage() {
  ...
  if (!circle) return <div className="pt-[68px] p-16 text-[var(--text-dim)] page-enter">Circle not found</div>
    ...
    <div className="page-enter min-h-screen pt-[68px]">
      <div className="bg-[var(--bg2)] border-b border-[var(--border)] px-16 py-6 relative overflow-hidden">
        <div className="flex items-center gap-4">
          ...
          <div className="text-[2rem]">{circle.emoji}</div>
          <div>
            <h1 className="font-syne font-extrabold text-2xl">{circle.name}</h1>
            <p className="text-sm text-[var(--text-dim)]">
              Created by {circle.creator} · 👥 {circle.members.toLocaleString()} members
            </p>
          </div>
          {isAdmin && (
            <div className="ml-auto px-3 py-1.5 text-xs rounded-[var(--radius-pill)] bg-[rgba(255,171,64,0.12)] text-[var(--accent-amber)] border border-[rgba(255,171,64,0.2)] font-medium">
              👑 Admin
            </div>
          )}
        </div>
              ...
              <div key={post.id} className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius)] p-5">
                <div className="flex items-start gap-3">
                  <div className={`w-11 h-11 rounded-full flex-shrink-0 flex items-center justify-center text-lg ${
                    post.isAdmin
                      ? 'bg-gradient-to-br from-[var(--teal)] to-[var(--accent-cyan)] ring-2 ring-[var(--teal-glow-strong)]'
        ...
        {isAdmin && (
          <div className="flex gap-3 items-center sticky bottom-6">
            ...
            <button onClick={handlePost}
              className="w-11 h-11 rounded-full bg-[var(--teal)] border-none flex items-center justify-center cursor-pointer transition-all hover:bg-[#00f5d0] hover:scale-105 flex-shrink-0 text-lg">
              ➤
            </button>
          </div>
        )}
        {!isAdmin && currentUserId && (
          <p className="text-center text-sm text-[var(--text-muted)] py-4">Only circle admins can post.</p>
        )}
      </div>
    </div>
  )
